...
import { Bell, AlertTriangle, CheckCircle, XCircle, Info, Plus, Settings, Trash2 } from 'lucide-react';
import { Card, CardContent } from '../components/ui/Card';
import Button from '../components/ui/Button';
import Modal from '../components/ui/Modal';
import Input, { Select } from '../components/ui/Input';
import Toggle from '../components/ui/Toggle';
import { alerts as initialAlerts, alertRules as initialRules, servers } from '../data/mockData';
      ...
      {activeTab === 'alerts' && (
        <div className="space-y-3">
          {alertList.map((alert) => {
            ...
            const server = servers.find(s => s.id === alert.server);
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <div className="flex items-center gap-3 mt-2 text-xs text-[#64748b]">
                            ...
                            <span>•</span>
                            <span>{getTimeAgo(new Date(alert.timestamp))}</span>
                          </div>
                        </div>
                        {!alert.read && (
                          <button
