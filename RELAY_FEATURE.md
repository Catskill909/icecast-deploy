...
## ROOT CAUSE FOUND: December 24, 2024 ~5:00 PM

**WHY `-fallback` mount ALWAYS fails:**

1. Static `icecast.xml` in repo has **NO MOUNTS** - just basic config
2. Container starts → Icecast reads empty config
3. Node.js starts 5 sec later → generates NEW config with mounts
4. Calls `reloadconfig` → **CAN'T ADD NEW MOUNTS** (Icecast limitation)
5. Result: `-fallback` mounts NEVER EXIST in running Icecast

**Why `/new` works:** Icecast accepts connections to ANY mount even without config. But `fallback-override` routing ONLY works if mounts are pre-defined at startup!

**The fix that was documented but NEVER applied:**

`startup.sh` exists and generates config BEFORE Icecast starts. But Dockerfile CMD bypasses it:
...
**THIS is what needs to change.** Not the relay code - the DOCKERFILE.

---

## Fix Attempt: December 24, 2024 ~5:02 PM

### Plan (Documented BEFORE applying)

**Change #1: Dockerfile (ONE LINE)**
- Line 61: Change `CMD ["supervisord", ...]` → `CMD ["/app/startup.sh"]`

**What startup.sh does:**
1. Generates icecast.xml with ALL mounts (including -fallback)
2. THEN starts supervisord (which starts Icecast + Node.js)

**Expected result:**
- Mounts exist BEFORE Icecast starts
- Fallback routing works
- Relay can connect to -fallback mount
- Mixxx can connect to main mount

**If this fails:**
- Document what error occurred
- Revert Dockerfile
- Accept limitation

### Applying fix now...

**Change applied:** Dockerfile line 61 now uses `CMD ["/app/startup.sh"]`
